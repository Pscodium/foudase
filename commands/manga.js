...
if(message.channel.type !== "text") return;
  ...
  if (!tag || !error54) return message.channel.send(`Para procurar mangá, você deve fazer: \`p;manga <Pesquisa>\``)
    request(`https://myanimelist.net/api/manga/search.xml?q=${tag}`, function (error, response, body) {
      if (error!=null) {
        message.channel.send(`❌ | Não foi possível encontrar nada para**${error54}**`);
      }else {
        ...
        parseString(body, function (err, result) {
            ...
            message.channel.send({embed:new Discord.RichEmbed()
              ...
              .setTitle(anime.title[0] + ` - Resultado da pesquisa Mangá`)
              .setURL(`https://myanimelist.net/anime/${anime.id[0]}`)
              .setImage(anime.image[0])
              .addField(`»Sinopse`, decode(anime.synopsis[0].replace(/<[^>]*>/g, ''), false).split('\n')[0])
              .addField(`»Tipo`,`${anime.type[0]}`,false)
              .addField(`»Capítulos`,`${anime.chapters[0]!='' ? anime.chapters[0] : '­'}`,true)
              //.addField(`External Link:`,`${anime.id[0]!='' ? `[MyAnimeList](https://myanimelist.net/anime/${anime.id[0]})` : '­'}`,true)
              .addField(`»Pontuação`,`${anime.score[0]!='' ? anime.score[0] : '­'}`,true)
              .addField(`»Status`,`${anime.status[0]}`,true)
              .addField(`»Volumes`,`${anime.volumes[0]!='' ? anime.volumes[0] : '­'}`,true)
              .addField(`»Data de início`, anime.start_date[0], true)
              .addField(`»Data de final`, anime.end_date[0], true)
