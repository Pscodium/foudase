...
module.exports.run = async (bot, message, args) => {
    ...
    const searchmessage = await message.channel.send('Pesquisando por ' + term);
      ...
      $.querySelectorAll('.r')
        ...
        .map(async (e) => {
          ...
          try {
            ...
            obj.title = details.title() || 'Nenhum título encontrado';
          }
          return obj;
        })
    ));
    ...
    if (!result.length) return message.channel.send('Nenhum resultado encontrado' + term);
    ...
    const embed = new Discord.RichEmbed()
      .setColor('RANDOM')
      .setAuthor(`Resultados para "${term}"`, 'https://lh4.googleusercontent.com/-v0soe-ievYE/AAAAAAAAAAI/AAAAAAADwkE/KyrKDjjeV1o/photo.jpg', searchurl)
    ...
    for (const [i, e] of embeds.entries()) {
      embed.addField(i === 0 ? 'Top Resultados' : '\u200b', e);
    }
    searchmessage.edit({
      embed
    });
